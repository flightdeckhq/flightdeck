"""LangChain smoke test. Runs manually; NOT in CI.
...
upgrade that changes the call shape. MCP coverage lives in this file
too: a ``langchain-mcp-adapters`` invocation must continue to route
through the patched ``ClientSession``.
"""
...
def test_langchain_openai_embeddings_capture_and_attribution() -> None:
    """LangChain ``OpenAIEmbeddings`` rides through the OpenAI patch
    transitively. Two assertions:

    1. ``has_content=True`` AND ``content.input`` round-trips intact
       (capture_prompts=True works through the LangChain wrapper).
    2. Per-session ``framework`` field reads ``"langchain"``, NOT
       ``"openai"`` — higher-level framework wins over the SDK
       transport.

    Also exercises the chat path symmetrically -- a ChatOpenAI invoke
    should produce framework=langchain too.
    """
    from langchain_openai import ChatOpenAI, OpenAIEmbeddings
    sess = _sensor_session()
    payload = "smoke langchain transitive"
    ...
    assert session_body.get("framework") == "langchain", (
        f"session must read framework=langchain; got "
        f"{session_body.get('framework')!r}"
    )
...
# ---------------------------------------------------------------------------
# MCP path: ``langchain-mcp-adapters`` wraps an MCP ``ClientSession``
# and exposes its tools as LangChain tools. The sensor patches
# ``ClientSession`` directly, so the adapter glue is transparent --
# every tool invocation through a LangChain agent still produces an
...
def test_langchain_mcp_adapter_routes_tool_calls_through_sensor() -> None:
    ...
    Skipped cleanly when ``langchain-mcp-adapters`` is not installed.
    """
