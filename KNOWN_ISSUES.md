...
| KI19 | Sensor     | CrewAI 1.14+ routes Anthropic traffic through litellm by default; litellm constructs its own HTTP layer rather than using the `anthropic` SDK classes, so the sensor's class-level `patch()` does not intercept it. OpenAI via CrewAI still works (native provider class). **v0.4.0 follow-up:** add a litellm interceptor to the sensor (covers many Python agent frameworks, not just CrewAI) OR document the limitation in README + plugin docs. Surfaced by `playground/06_crewai.py`, which runs the OpenAI path for this reason. | D089      |
