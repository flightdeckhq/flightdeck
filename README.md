...
- **Symmetric `/assets/` cache-discipline smoke.** The `dashboard-image-smoke` CI job covers `/runtime-config.json`'s strict no-store policy. The opposite invariant — `/assets/` shipping `Cache-Control: public, immutable` — has no equivalent regression backstop today. Same pattern (build prod image, curl, assert headers), different file's discipline. Worth a follow-up commit to the smoke job once a user reports a stale-bundle issue or a reviewer flags the asymmetry.
