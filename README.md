...
The chart is at `version` 0.5.2 with `appVersion` 0.5.2; the default `image.tag` is `v0.5.2`. See [`helm/values.yaml`](helm/values.yaml) for the full schema, including replicas, HPA bounds, ingress, resources, and security contexts.

| Key | Default | Description |
|---|---|---|
| `image.tag` | `v0.5.2` | Image tag applied to ingestion/workers/api/dashboard. |
