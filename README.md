...
- **Symmetric `/assets/` cache-discipline smoke.** The Dashboard CI job's production-image smoke steps cover `/runtime-config.json`'s strict no-store policy. The opposite invariant — `/assets/` shipping `Cache-Control: public, immutable` — has no equivalent regression backstop today. Same pattern (build prod image, curl, assert headers), different file's discipline. Worth a follow-up commit to the smoke steps once a user reports a stale-bundle issue or a reviewer flags the asymmetry.
- **Workflow-level least-privilege `permissions:` declaration.** `.github/workflows/ci.yml` has no top-level `permissions:` block; every job inherits GitHub's default token scope which is broader than any job actually needs. A workflow-level `permissions: contents: read` default with per-job overrides where writes are needed (e.g. artifact upload on `e2e`) closes the supply-chain gap that a future compromised third-party action would otherwise widen. Pre-existing posture; flag before the next workflow change.
