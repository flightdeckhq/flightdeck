...
client-side at ``call_tool`` invocation time. The block path
synchronously flushes the event queue before raising so the block
event arrives at the dashboard before the agent sees the failure.
...
async def run_demo() -> int:
    flavor = f"playground-mcp-policy-block-{uuid.uuid4().hex[:6]}"
    ...
    # 2. Provision a flavor policy: deny+block against the matched
    # server. Flavor entry takes precedence (D135 step 1).
    ...
    # flavor entry, decides warn → POLICY_MCP_BLOCK emits and the
    # tool call proceeds.
    ...
    async with _mcp_stdio.stdio_client(params) as (read, write):  # noqa: SIM117
        async with ClientSession(read, write) as session:
            ...
            try:
                ...
                blocked_exc = exc
    assert blocked_exc is not None, "expected MCPPolicyBlocked to be raised"
    assert blocked_exc.fingerprint, "fingerprint missing on exception"
    assert blocked_exc.decision_path == "flavor_entry", (
        f"unexpected decision_path: {blocked_exc.decision_path}"
    )
...
def main() -> int:
    try:
        ...
        print_result("mcp-policy-block", False, 0, str(exc))
        return 2
    t0 = time.monotonic()
    try:
        ...
        print_result(
            ...
            rc == 0,
            duration_ms,
            "policy_mcp_block event landed; flavor entry deny+warn enforcement",
        )
        return rc
    except AssertionError as exc:
        duration_ms = int((time.monotonic() - t0) * 1000)
        print_result("mcp-policy-block", False, duration_ms, str(exc))
        return 1
