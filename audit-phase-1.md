...
2. **Integration tests not migrated to D115 vocabulary.** `tests/integration/conftest.py`, `test_session_states.py`, and `test_ui_demo.py` still emit `agent_type="autonomous"` / `"developer"` / name `"supervised"` and `"batch"` in expected-value assertions. The ingestion validator will reject these payloads at runtime, and the state-transition assertions will fail on the `"unknown" → "autonomous"` upgrade expectation. These suites do not run in the phase 1 CI — `make test-integration` is a separate gate — but the next operator who runs `make test-integration` or `pytest tests/integration/test_ui_demo.py -v -s` will hit failures. Recommend a follow-up PR that migrates the integration suite before the v0.4.0 tag.
