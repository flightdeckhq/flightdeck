...
| KI26 | Sensor     | SensorLitellm (KI21) does not yet support `stream=True`. The patched `litellm.completion` / `litellm.acompletion` wrappers raise `NotImplementedError` with a pointer to this KI when streaming is requested. Streaming support requires a parallel implementation of the sensor's `GuardedStream` for litellm's `ModelResponseStream` chunks -- tokens accumulate across chunks, usage only surfaces on the terminal chunk, and the pre/post-call event pair has to bookend the whole iteration rather than a single call. Non-streaming `completion` / `acompletion` cover the core KI21 bypass case; streaming is a real follow-up, not vaporware. **v0.4.0 follow-up:** implement `base.call_stream` for litellm ModelResponseStream -- mirror `interceptor/openai.py`'s `_inject_stream_options` + `SensorCompletions.create(stream=True)` path, accumulating usage from the last chunk and emitting the post-call event on stream completion. | —         |
